...
namespace vkglTF {
    ...
    void Texture::fromglTfImage(tinygltf::Image &gltfimage, TextureSampler textureSampler, vks::VulkanDevice *device,
                                VkQueue copyQueue) {
        this->device = device;
    ...
    Mesh::Mesh(vks::VulkanDevice *device, glm::mat4 matrix) {
        ...
        VK_CHECK_RESULT(device->createBuffer(
                ...
                &uniformBlock))
        VK_CHECK_RESULT(vkMapMemory(device->logicalDevice, uniformBuffer.memory, 0, sizeof(uniformBlock), 0,
            ...
            for (const auto & primitive : mesh.primitives) {
                ...
                // Vertices
                {
                    ...
                    if (primitive.attributes.find("COLOR_0") != primitive.attributes.end()) {
                        const tinygltf::Accessor &accessor = model.accessors[primitive.attributes.find(
                                "COLOR_0")->second];
                    ...
                    for (size_t v = 0; v < posAccessor.count; v++) {
                        ...
                        vert.uv1 = bufferTexCoordSet1 ? glm::make_vec2(&bufferTexCoordSet1[v * uv1ByteStride])
                                                      : glm::vec3(0.0f);
                        vert.color = bufferColorSet0 ? glm::make_vec4(&bufferColorSet0[v * color0ByteStride])
                                                     : glm::vec4(1.0f);
    ...
    void Model::getNodeProps(const tinygltf::Node &node, const tinygltf::Model &model, size_t &vertexCount,
                             size_t &indexCount) {
        if (node.children.size() > 0) {
