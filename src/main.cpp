...
public:
    ...
    HelloTriangleApplication() : VulkanExampleBase() {
        title = "CGGD : gltf loading";
    ...
    ~HelloTriangleApplication() {
        for (auto &pipeline: pipelines) vkDestroyPipeline(device, pipeline.second, nullptr);
    ...
    void renderNode(vkglTF::Node *node, uint32_t cbIndex, vkglTF::Material::AlphaMode alphaMode) {
        if (node->mesh) {
            for (vkglTF::Primitive *primitive: node->mesh->primitives) {
                if (primitive->material.alphaMode == alphaMode) {
                    ...
                    if (primitive->material.unlit) pipelineName = "unlit";
                    ...
                    vkCmdBindDescriptorSets(commandBuffers[cbIndex], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0,
                    ...
                    vkCmdPushConstants(commandBuffers[cbIndex], pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                       sizeof(uint32_t), &primitive->material.index);

                    if (primitive->hasIndices) {
                        vkCmdDrawIndexed(commandBuffers[cbIndex], primitive->indexCount, 1, primitive->firstIndex, 0,
                                         0);
                    } else {
        ...
        for (auto child: node->children) renderNode(child, cbIndex, alphaMode);
    }
    ...
    void createMaterialBuffer() {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                                   ...
                                                   VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, bufferSize,
                                                   &stagingBuffer.buffer, &stagingBuffer.memory,
                                                   shaderMaterials.data()))
        VK_CHECK_RESULT(
                vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
        ...
        for (auto &arg: args) {
            if ((std::string(arg).find(".gltf") != std::string::npos) ||
                ...
                if (file.good()) {
                    ...
                    std::cout << "could not load \"" << arg << "\"" << std::endl;
                }
            }
    ...
    void setupNodeDescriptorSet(vkglTF::Node *node) {
        ...
        for (auto &child: node->children) setupNodeDescriptorSet(child);
    }
    ...
    void setupDescriptors() {
        ...
        */
        ...
        // Material (samplers)
        {
            ...
            descriptorSetLayoutCI.pBindings = setLayoutBindings.data();
            descriptorSetLayoutCI.bindingCount = static_cast<uint32_t>(setLayoutBindings.size());
            VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device, &descriptorSetLayoutCI, nullptr,
    ...
    */
    void prepareUniformBuffers() {
        for (auto &uniformBuffer: uniformBuffers) {
            ...
            uniformBuffer.skybox.create(vulkanDevice, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                        sizeof(shaderValuesSkybox));
            uniformBuffer.params.create(vulkanDevice, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
    ...
    */
    void updateOverlay() {
        ...
        if (imDrawData) {
            ...
            bool updateBuffers = (ui->vertexBuffer.buffer == VK_NULL_HANDLE) ||
            ...
            if (updateBuffers) {
                ...
                ui->vertexBuffer.create(vulkanDevice, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, vertexBufferSize);
                ui->vertexBuffer.count = imDrawData->TotalVtxCount;
                if (ui->indexBuffer.buffer) {
                    ui->indexBuffer.destroy();
                }
                ui->indexBuffer.create(vulkanDevice, VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
...
HelloTriangleApplication *vulkanExample;
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int) {
    for (int32_t i = 0; i < __argc; i++) { HelloTriangleApplication::args.push_back(__argv[i]); }
    vulkanExample = new HelloTriangleApplication();
